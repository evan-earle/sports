...
import { fetchNFLNews } from "../../../actions/index";
...
class Home extends React.Component {
  ...
  render() {
    ...
    } else if (this.props.NFLNews.articles) {
      const author = this.props.NFLNews.articles.map(
        (element) => element.source.name
      );

      const title = this.props.NFLNews.articles.map(
        (element) => element.title.split(" - ")[0]
      );

      const time = this.props.NFLNews.articles.map(
        (element) => element.publishedAt
      );

      const timestamp = time.map((element) => moment(element).fromNow());

      const newsImage = this.props.NFLNews.articles.map(
        (element) => element.urlToImage
      );

      const articleURL = this.props.NFLNews.articles.map(
        (element) => element.url
      );
...
const mapStateToProps = (state) => {
  return {
    NFLNews: state.NFLNews.news,
    loading: state.NFLNews.loading,
  };
};
